...
# Important Concepts:
# -------------------
# The Screen Buffer is an array of characters and color data used for output.
# Its size can be much larger than the visible display dimensions.
...
function Debug
{
    param ([object]$debugMsg = "",
	       [switch]$beginFunction = $false,
		   [switch]$endFunction = $false,
		   [switch]$errorOccurred = $false)
    
    if ($debugFlag -ne $true) { return }
		
	if ($debugMsg -eq $null) { $debugMsg = "" }
    	
	if ($beginFunction)
	{
		$callingCmd = (Get-PSCallStack)[1].Command
		Debug "$([string]::New('v', 80))"
		Debug "Begin Function: $callingCmd"
		Debug "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"

	    $parms = (Get-Command -Name $callingCmd).Parameters		
		Debug "Parameters (Count: $($parms.Count)) :"
		
		if ($parms.Count -gt 0)
		{
			foreach ($parm in $parms)
				{ Debug (Get-Variable -Name $parm.Values.Name -ErrorAction SilentlyContinue) }
		}
		return
	}
    ...
    if ($debugMsg.GetType().Name -eq "String")
		{ $msgOut += $debugMsg }
    else
		{ $msgOut += $($debugMsg | Out-String) }

	Add-Content -Path $debugFile -Value $msgOut
}

# Gets run once at import time.
if ($debugFlag -eq $true)
{
	$debugDir = Split-Path -Parent $debugFile
    if ((Test-Path -Path $debugDir) -eq $false)
...
function Get-ScreenInfo
{
	...
	$screenInfo = @{"BufferSize" = $Host.UI.RawUI.BufferSize;
...
{
	param ($screenInfo) # Saved values from call to Get-ScreenInfo.

	Debug -beginFunction

	[Console]::ForegroundColor = $screenInfo["ForegroundColor"]
	[Console]::BackgroundColor = $screenInfo["BackgroundColor"]

	Debug "Screen Colors After Restore:"    
	Debug "[Console]::ForegroundColor=$([Console]::ForegroundColor)"
	Debug "[Console]::BackgroundColor=$([Console]::BackgroundColor)"
...
#-----------------------------------------------------------------------------
function Restore-CursorLocation
{
	param ($screenInfo) # Saved values from call to Get-ScreenInfo.

	Debug -beginFunction

    $Host.UI.RawUI.CursorPosition = $screenInfo["CursorPosition"]

	Debug "Cursor Location After Restore:"
	Debug $Host.UI.RawUI.CursorPosition
...
function Show-Box
{
    ...
    #       X and Y are bounds limited to the visible window.
    #   - Width, Height
    #       Box width and height.
    #       If Width is not supplied, defaults to 10.
    #       If Height is not supplied, defaults to 10.
    #       Box X+Width and Y+Height must fit within the visible window.
    #   - ForegroundColor, BackgroundColor
    #       ForegroundColor is line color, BackgroundColor is box color.
    #       Valid color names are any of the System.ConsoleColor values.
	#       https://learn.microsoft.com/en-us/dotnet/api/system.consolecolor?view=net-9.0
    #   - LineStyle
    ...
    #       when the function was called.  Otherwise, it will be left in the first position
	#       on the next line.
    ...
    param ([int]$X = 0,
           ...
           [int]$Width = 10,
           [int]$Height = 10,
           [string]$ForegroundColor = "Cyan", #([Console]::ForegroundColor).ToString(),
           [string]$BackgroundColor = "DarkBlue", #([Console]::BackgroundColor).ToString(),
    ...
    try
    {
		$savedScreenInfo = Get-ScreenInfo

        $Host.UI.RawUI.ForegroundColor = $ForegroundColor
        $Host.UI.RawUI.BackgroundColor = $BackgroundColor
        
		Debug "New colors:"
		Debug "    ForegroundColor=$([Console]::ForegroundColor)"
		Debug "    BackgroundColor=$([Console]::BackgroundColor)"


        Debug "Bounds Checking ..."
		
        if ($X -lt 0)
			{ throw "Show-Box: Parameter out of bounds.  X=$X.  X cannot be less than 0." }
        if ($Width -lt 3)
			{ throw "Show-Box: Parameter out of bounds.  Width=$Width.  Width cannot be less than 3." }
        if (($X + $Width) -gt $Host.UI.RawUI.WindowSize.Width)
			{ throw "Show-Box: Parameters out of bounds.  (X=$X) + (Width=$Width) = $($X+$Width).  X + Width cannot be greater than right column of the console window ($($Host.UI.RawUI.WindowSize.Width))." }
        if ($Y -lt 0)
			{ throw "Show-Box: Parameter out of bounds.  Y=$Y.  Y cannot be less than 0." }
        if ($Height -lt 3)
			{ throw "Show-Box: Parameter out of bounds.  Height=$Height.  Height cannot be less than 3." }
        if (($Y + $Height) -gt $Host.UI.RawUI.WindowSize.Height)
			{ throw "Show-Box: Parameters out of bounds.   (Y=$Y) + (Height=$Height) = $($Y+$Height).  Y + Height cannot be greater than the bottom row of the console window ($($Host.UI.RawUI.WindowSize.Height))." }
        ...
        $midLinesCount = $Height-2
        Debug "    midLinesCount=$midLinesCount"

        for ($midLineNbr = 1; $midLineNbr -le $midLinesCount; $midLineNbr++)
        {
        ...
        Debug "Restoring colors ..."
		Restore-Colors  $savedScreenInfo
		
        if ($SaveCursorLocation)
		{
		    Debug "Restoring CursorLocation ..."
			Restore-CursorLocation  $savedScreenInfo
		}
		else
		{
			[Console]::Write("`r`n")
    ...
    {
        Debug $_ -errorOccurred
		
		Debug "Restoring colors ..."
		Restore-Colors $savedScreenInfo

        Debug "Re-throwing exception ..."
        throw $_.Exception
    }
...
function Show-TextBox
{
    ...
    #       Default is Center.
    #   - HorizontalAlign
    #       Box will be aligned horizontally as Left, Center, or Right of.
    #       screen.  Default is Center.
    #   - VerticalAlign
    #       Box will be aligned vertically as Top, Middle, or Bottom of
    #       screen.  Default is Middle.
    #   - X, Y
    ...
    param ([string]$Text,
           [TextJustify]$Justify = ([TextJustify]::Left).ToString(),
           [HorizontalAlign]$HorizontalAlign = ([HorizontalAlign]::Center).ToString(),
           [VerticalAlign]$VerticalAlign = ([VerticalAlign]::Middle).ToString(),
           [int]$X = -1,
           [int]$Y = -1,
           [int]$Width = -1,
           [int]$Height = -1,
           ...
           [TextJustify]$TitleJustify = ([TextJustify]::Left).ToString(),
           [int]$PadWidth = 1,
           [int]$PadHeight = 0)
    ...
    {
        Debug "Show-MessageBox: Exception occurred"
        Debug $_
        $Host.UI.RawUI.ForegroundColor = $saveFgColor
        $Host.UI.RawUI.BackgroundColor = $saveBgColor
        throw $_.Exception
