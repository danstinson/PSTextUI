...
function Debug
{
    ...
    if ($debugFlag -ne $true)
		{ return }
		
	if ($debugMsg -eq $null)
		{ $debugMsg = "" }
    
	if ($debugMsg -eq "BEGIN_FUNCTION")
	{
		$cmd = (Get-PSCallStack)[1].Command
		$args = (Get-PSCallStack)[1].Arguments
		Debug "Begin Function: $cmd $([string]::New('-', 70))"
		Debug "Arguments: $args"
		Debug "All Parameter Values:"
		foreach ($parm in (Get-Command -Name $cmd).Parameters)
			{ $allParms += Get-Variable -Name $parm.Values.Name -ErrorAction SilentlyContinue }
        Debug $allParms
		Debug "SHOW_SCREEN_DIMENSIONS"
		return
	}

	if ($debugMsg -eq "SHOW_SCREEN_DIMENSIONS")
	{
		$screenDim = @{"Host.UI.RawUI.BufferSize" = $Host.UI.RawUI.BufferSize;
		               "Host.UI.RawUI.WindowSize" = $Host.UI.RawUI.WindowSize;
					   "Host.UI.RawUI.WindowPosition" = $Host.UI.RawUI.WindowPosition;
	...
	if ($debugMsg -eq "END_FUNCTION")
	{
		$cmd = (Get-PSCallStack)[1].Command
		Debug "End Function: $cmd $([string]::New('-', 70))"
		return
	}
	
    $msgOut = "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - "
...
#-----------------------------------------------------------------------------
function Save-PositionAndColors
{
	Debug "BEGIN_FUNCTION"

    $savedInfo = @{	"CursorPosition" = $Host.UI.RawUI.CursorPosition;
...
#-----------------------------------------------------------------------------
function Restore-PositionAndColors
{
	param ($savedInfo)
	
	Debug "BEGIN_FUNCTION"

    $Host.UI.RawUI.CursorPosition = $savedInfo["CursorPosition"]
	[Console]::ForegroundColor = $savedInfo["ForegroundColor"]
	[Console]::BackroundColor = $savedInfo["BackroundColor"]
...
function Show-Box
{
    ...
    try
    {
		$savedInfo = Save-PositionAndColors
